  ...
  export default{
    ...
    methods:{
      loadDataByPage:function(){
          var url="http://127.0.0.1/12-html5/admin/data/04_product_list.php";
          url+=`?pno=${this.nowPage}&pageSize=${this.pageSize}`;
      ...
      switchPage:function (rs) {
          if(rs==false && this.nowPage>=1){
            this.nowPage--;
          }else if(rs==true && this.nowPage<=this.totalPages){
              this.nowPage++;
          }
          this.loadDataByPage(this.nowPage);
      }
    },
    mounted(){
